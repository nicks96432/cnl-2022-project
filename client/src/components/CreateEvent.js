      ...
      <div>
        <Form>
          <Row>
            ...
            <InputGroup>
              <DateRangePicker minDate={startDate} ranges={[dateRange]} onChange={handleDate}></DateRangePicker>
            </InputGroup>
            <Button variant="outline-secondary">Today</Button>
          </Row>
         <Form.Label>What times might work?</Form.Label>
          <DropdownButton id="dropdown-basic-button" title="No earlier than:" onSelect={handleStartTime}>
            {time_choices.map((time) => (
              <Dropdown.Item eventKey={time}>{time}:00</Dropdown.Item>
            ))}
          </DropdownButton>
          <DropdownButton id="dropdown-basic-button" title="No later than:" onSelect={handleEndTime}>
            {time_choices.map((time) => (
              <Dropdown.Item eventKey={time}>{time}:00</Dropdown.Item>
            ))}
          </DropdownButton>
